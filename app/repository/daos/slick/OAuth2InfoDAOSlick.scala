    ...
    Future.successful(
      DB withSession { implicit session =>
        ...
        slickOAuth2Infos.filter(_.loginInfoId === infoId).firstOption match {
          case Some(info) =>
            slickOAuth2Infos.filter(_.loginInfoId === infoId) update DBOAuth2Info(info.id, authInfo.accessToken, authInfo.tokenType, authInfo.expiresIn, authInfo.refreshToken, infoId)
